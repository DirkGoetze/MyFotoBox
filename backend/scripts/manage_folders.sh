...
get_nginx_conf_dir() {
    ...
    debug "$get_nginx_conf_dir_txt_0001" "CLI" "get_nginx_conf_dir"
    if [ -n "$CONF_DIR_NGINX" ] && [ -d "$CONF_DIR_NGINX" ]; then
        debug "$(printf "$get_nginx_conf_dir_txt_0002" "$CONF_DIR_NGINX")" "CLI" "get_nginx_conf_dir"
    ...
    # Verwende die in lib_core definierten Pfade
    debug "$get_nginx_conf_dir_txt_0003" "CLI" "get_nginx_conf_dir"
    dir=$(get_folder_path "$DEFAULT_DIR_CONF_NGINX" "$FALLBACK_DIR_CONF_NGINX" 1)
    if [ -n "$dir" ]; then
        debug "$(printf "$get_nginx_conf_dir_txt_0004" "$dir")" "CLI" "get_nginx_conf_dir"
        # Setze explizit die Standard-Berechtigungen (755)
        create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
        echo "$dir"
        return 0
    ...
    debug "$(printf "$get_nginx_conf_dir_txt_0005" "$dir")" "CLI" "get_nginx_conf_dir"
    # Setze explizit die Standard-Berechtigungen (755)
    create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
    echo "$dir"
    return 0
...
# get_https_conf_dir
get_https_conf_dir_txt_0001="Ermittle HTTPS-Konfigurations-Verzeichnis"
get_https_conf_dir_txt_0002="Verwende bereits definiertes CONF_DIR_HTTPS: %s" 
get_https_conf_dir_txt_0003="Prüfe Standard- und Fallback-Pfade für HTTPS-Konfigurations-Verzeichnis"
get_https_conf_dir_txt_0004="Verwende Pfad für HTTPS-Konfigurations-Verzeichnis: %s"
get_https_conf_dir_txt_0005="Fallback für HTTPS-Konfigurations-Verzeichnis: %s"
...
get_https_conf_dir() {
    ...
    debug "$get_https_conf_dir_txt_0001" "CLI" "get_https_conf_dir"
    if [ -n "$CONF_DIR_HTTPS" ] && [ -d "$CONF_DIR_HTTPS" ]; then
        debug "$(printf "$get_https_conf_dir_txt_0002" "$CONF_DIR_HTTPS")" "CLI" "get_https_conf_dir"
        # Setze explizit die Standard-Berechtigungen (755)
    ...
    # Verwende die in lib_core definierten Pfade
    debug "$get_https_conf_dir_txt_0003" "CLI" "get_https_conf_dir"
    dir=$(get_folder_path "$DEFAULT_DIR_CONF_HTTPS" "$FALLBACK_DIR_CONF_HTTPS" 1)
    if [ -n "$dir" ]; then
        debug "$(printf "$get_https_conf_dir_txt_0004" "$dir")" "CLI" "get_https_conf_dir"
        # Setze explizit die Standard-Berechtigungen (755)
        create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
        echo "$dir"
        return 0
    ...
    debug "$(printf "$get_https_conf_dir_txt_0005" "$dir")" "CLI" "get_https_conf_dir"
    # Setze explizit die Standard-Berechtigungen (755)
    create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
    echo "$dir"
    return 0
...
# get_camera_conf_dir
get_camera_conf_dir_txt_0001="Ermittle Kamera-Konfigurations-Verzeichnis"
get_camera_conf_dir_txt_0002="Verwende bereits definiertes CONF_DIR_CAMERA: %s"
get_camera_conf_dir_txt_0003="Prüfe Standard- und Fallback-Pfade für Kamera-Konfigurations-Verzeichnis"
get_camera_conf_dir_txt_0004="Verwende Pfad für Kamera-Konfigurations-Verzeichnis: %s"
get_camera_conf_dir_txt_0005="Fallback für Kamera-Konfigurations-Verzeichnis: %s"
...
get_camera_conf_dir() {
    ...
    debug "$get_camera_conf_dir_txt_0001" "CLI" "get_camera_conf_dir"
    if [ -n "$CONF_DIR_CAMERA" ] && [ -d "$CONF_DIR_CAMERA" ]; then
        debug "$(printf "$get_camera_conf_dir_txt_0002" "$CONF_DIR_CAMERA")" "CLI" "get_camera_conf_dir"
        # Setze explizit die Standard-Berechtigungen (755)
    ...
    # Verwende die in lib_core definierten Pfade
    debug "$get_camera_conf_dir_txt_0003" "CLI" "get_camera_conf_dir"
    ...
    if [ -n "$dir" ]; then
        debug "$(printf "$get_camera_conf_dir_txt_0004" "$dir")" "CLI" "get_camera_conf_dir"
        # Setze explizit die Standard-Berechtigungen (755)
        create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
        echo "$dir"
        return 0
    ...
    debug "$(printf "$get_camera_conf_dir_txt_0005" "$dir")" "CLI" "get_camera_conf_dir"
    # Setze explizit die Standard-Berechtigungen (755)
    create_directory "$dir" "$DEFAULT_USER" "$DEFAULT_GROUP" "$DEFAULT_MODE" || true
...
# get_data_dir
get_data_dir_txt_0001="Ermittle Daten-Verzeichnis"
get_data_dir_txt_0002="Verwende bereits definiertes DATA_DIR: %s"
get_data_dir_txt_0003="Prüfe Standard- und Fallback-Pfade für Daten-Verzeichnis"
get_data_dir_txt_0004="Verwende Pfad für Daten-Verzeichnis: %s"
get_data_dir_txt_0005="Alle Pfade für Daten-Verzeichnis fehlgeschlagen, verwende %s/data"
...
get_data_dir() {
    ...
    debug "$get_data_dir_txt_0001" "CLI" "get_data_dir"
    if [ -n "$DATA_DIR" ] && [ -d "$DATA_DIR" ]; then
        debug "$(printf "$get_data_dir_txt_0002" "$DATA_DIR")" "CLI" "get_data_dir"
    ...
    # Verwende die in dieser Datei definierten Pfade
    debug "$get_data_dir_txt_0003" "CLI" "get_data_dir"
    dir=$(get_folder_path "$DEFAULT_DIR_DATA" "$FALLBACK_DIR_DATA" 1)
    if [ -n "$dir" ]; then
        debug "$(printf "$get_data_dir_txt_0004" "$dir")" "CLI" "get_data_dir"
        echo "$dir"
        return 0
    fi
    ...
    local install_dir
    install_dir=$(get_install_dir)
    debug "$(printf "$get_data_dir_txt_0005" "$install_dir")" "CLI" "get_data_dir"
...
# get_frontend_dir
get_frontend_dir_txt_0001="Ermittle Frontend-Verzeichnis"
get_frontend_dir_txt_0002="Verwende bereits definiertes FRONTEND_DIR: %s"
get_frontend_dir_txt_0003="Prüfe Standard-Frontend-Verzeichnis: %s"
get_frontend_dir_txt_0004="Standard-Frontend-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_frontend_dir() {
    ...
    debug "$get_frontend_dir_txt_0001" "CLI" "get_frontend_dir"
    if [ -n "$FRONTEND_DIR" ] && [ -d "$FRONTEND_DIR" ]; then
        debug "$(printf "$get_frontend_dir_txt_0002" "$FRONTEND_DIR")" "CLI" "get_frontend_dir"
    ...
    debug "$(printf "$get_frontend_dir_txt_0003" "$dir")" "CLI" "get_frontend_dir"
    if [ -d "$dir" ]; then
        create_directory "$dir" || true
        echo "$dir"
        return 0
    ...
    debug "$(printf "$get_frontend_dir_txt_0004" "$dir")" "CLI" "get_frontend_dir"
    create_directory "$dir" || true
...
get_frontend_css_dir_txt_0001="Ermittle Frontend-CSS-Verzeichnis"
get_frontend_css_dir_txt_0002="Prüfe CSS-Verzeichnis: %s"
get_frontend_css_dir_txt_0003="Verwende CSS-Verzeichnis: %s"
get_frontend_css_dir_txt_0004="CSS-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_frontend_css_dir() {
    ...
    # -----------------------------------------------------------------------
    local frontend_dir
    debug "$get_frontend_css_dir_txt_0001" "CLI" "get_frontend_css_dir"
    
    # Hole das Frontend-Hauptverzeichnis
    frontend_dir=$(get_frontend_dir)
    local css_dir="$frontend_dir/css"
    
    debug "$(printf "$get_frontend_css_dir_txt_0002" "$css_dir")" "CLI" "get_frontend_css_dir"
    if create_directory "$css_dir"; then
        debug "$(printf "$get_frontend_css_dir_txt_0003" "$css_dir")" "CLI" "get_frontend_css_dir"
        echo "$css_dir"
        return 0
    fi
    
    # Fallback direkt zum Frontend-Verzeichnis
    debug "$(printf "$get_frontend_css_dir_txt_0004" "$frontend_dir")" "CLI" "get_frontend_css_dir"
    echo "$frontend_dir"
    return 1
}

# get_frontend_fonts_dir
get_frontend_fonts_dir_txt_0001="Ermittle Frontend-Fonts-Verzeichnis"
get_frontend_fonts_dir_txt_0002="Prüfe Fonts-Verzeichnis: %s"
get_frontend_fonts_dir_txt_0003="Verwende Fonts-Verzeichnis: %s"
get_frontend_fonts_dir_txt_0004="Fonts-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_frontend_fonts_dir() {
    ...
    # -----------------------------------------------------------------------
    local frontend_dir
    debug "$get_frontend_fonts_dir_txt_0001" "CLI" "get_frontend_fonts_dir"
    
    # Hole das Frontend-Hauptverzeichnis
    frontend_dir=$(get_frontend_dir)
    local fonts_dir="$frontend_dir/fonts"
    
    debug "$(printf "$get_frontend_fonts_dir_txt_0002" "$fonts_dir")" "CLI" "get_frontend_fonts_dir"
    if create_directory "$fonts_dir"; then
        debug "$(printf "$get_frontend_fonts_dir_txt_0003" "$fonts_dir")" "CLI" "get_frontend_fonts_dir"
        echo "$fonts_dir"
        return 0
    fi
    
    # Fallback direkt zum Frontend-Verzeichnis
    debug "$(printf "$get_frontend_fonts_dir_txt_0004" "$frontend_dir")" "CLI" "get_frontend_fonts_dir"
    echo "$frontend_dir"
    return 1
}

# get_frontend_js_dir
get_frontend_js_dir_txt_0001="Ermittle Frontend-JavaScript-Verzeichnis"
get_frontend_js_dir_txt_0002="Prüfe JavaScript-Verzeichnis: %s"
get_frontend_js_dir_txt_0003="Verwende JavaScript-Verzeichnis: %s"
get_frontend_js_dir_txt_0004="JavaScript-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_frontend_js_dir() {
    ...
    # -----------------------------------------------------------------------
    local frontend_dir
    debug "$get_frontend_js_dir_txt_0001" "CLI" "get_frontend_js_dir"
    
    # Hole das Frontend-Hauptverzeichnis
    frontend_dir=$(get_frontend_dir)
    local js_dir="$frontend_dir/js"
    
    debug "$(printf "$get_frontend_js_dir_txt_0002" "$js_dir")" "CLI" "get_frontend_js_dir"
    if create_directory "$js_dir"; then
        debug "$(printf "$get_frontend_js_dir_txt_0003" "$js_dir")" "CLI" "get_frontend_js_dir"
        echo "$js_dir"
        return 0
    fi
    
    # Fallback direkt zum Frontend-Verzeichnis
    debug "$(printf "$get_frontend_js_dir_txt_0004" "$frontend_dir")" "CLI" "get_frontend_js_dir"
    echo "$frontend_dir"
    return 1
}

# get_photos_dir
get_photos_dir_txt_0001="Ermittle Fotos-Verzeichnis"
get_photos_dir_txt_0002="Prüfe Standard-Fotos-Verzeichnis: %s"
get_photos_dir_txt_0003="Verwende Standard-Fotos-Verzeichnis: %s"
get_photos_dir_txt_0004="Standard-Fotos-Verzeichnis nicht verfügbar, verwende Fallback: %s/photos"
...
get_photos_dir() {
    ...
    # Rückgabe: Pfad zum Verzeichnis oder leerer String bei Fehler
    # -----------------------------------------------------------------------
    debug "$get_photos_dir_txt_0001" "CLI" "get_photos_dir"
    
    local frontend_dir
    frontend_dir=$(get_frontend_dir)
    
    local photos_dir="$frontend_dir/photos"
    debug "$(printf "$get_photos_dir_txt_0002" "$photos_dir")" "CLI" "get_photos_dir"
    if create_directory "$photos_dir"; then
        debug "$(printf "$get_photos_dir_txt_0003" "$photos_dir")" "CLI" "get_photos_dir"
        echo "$photos_dir"
        return 0
    fi
    ...
    debug "$(printf "$get_photos_dir_txt_0004" "$data_dir")" "CLI" "get_photos_dir"
    echo "$data_dir/photos"
    return 0
}

# get_photos_originals_dir
get_photos_originals_dir_txt_0001="Ermittle Original-Fotos-Verzeichnis"
get_photos_originals_dir_txt_0002="Prüfe Originals-Verzeichnis: %s"
get_photos_originals_dir_txt_0003="Event-Name angegeben, prüfe Verzeichnis: %s"
get_photos_originals_dir_txt_0004="Verwende Event-spezifisches Original-Fotos-Verzeichnis: %s"
get_photos_originals_dir_txt_0005="Verwende Standard Original-Fotos-Verzeichnis: %s"
get_photos_originals_dir_txt_0006="Originals-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_photos_originals_dir() {
    ...
    # -----------------------------------------------------------------------
    local event_name="$1"
    debug "$get_photos_originals_dir_txt_0001" "CLI" "get_photos_originals_dir"
    
    local photos_dir
    photos_dir=$(get_photos_dir)
    local originals_dir="$photos_dir/originals"
    
    debug "$(printf "$get_photos_originals_dir_txt_0002" "$originals_dir")" "CLI" "get_photos_originals_dir"
    if create_directory "$originals_dir"; then
        if [ -n "$event_name" ]; then
            local event_dir="$originals_dir/$event_name"
            debug "$(printf "$get_photos_originals_dir_txt_0003" "$event_dir")" "CLI" "get_photos_originals_dir"
            if create_directory "$event_dir"; then
                debug "$(printf "$get_photos_originals_dir_txt_0004" "$event_dir")" "CLI" "get_photos_originals_dir"
                echo "$event_dir"
                return 0
            fi
        else
            debug "$(printf "$get_photos_originals_dir_txt_0005" "$originals_dir")" "CLI" "get_photos_originals_dir"
            echo "$originals_dir"
            return 0
        fi
    fi
    ...
    debug "$(printf "$get_photos_originals_dir_txt_0006" "$photos_dir")" "CLI" "get_photos_originals_dir"
    echo "$photos_dir"
    return 0
}

# get_photos_gallery_dir
get_photos_gallery_dir_txt_0001="Ermittle Galerie-Verzeichnis"
get_photos_gallery_dir_txt_0002="Prüfe Galerie-Verzeichnis: %s"
get_photos_gallery_dir_txt_0003="Event-Name angegeben, prüfe Verzeichnis: %s"
get_photos_gallery_dir_txt_0004="Verwende Event-spezifisches Galerie-Verzeichnis: %s"
get_photos_gallery_dir_txt_0005="Verwende Standard Galerie-Verzeichnis: %s"
get_photos_gallery_dir_txt_0006="Galerie-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_photos_gallery_dir() {
    ...
    # -----------------------------------------------------------------------
    local event_name="$1"
    debug "$get_photos_gallery_dir_txt_0001" "CLI" "get_photos_gallery_dir"
    
    local photos_dir
    photos_dir=$(get_photos_dir)
    local gallery_dir="$photos_dir/gallery"
    
    debug "$(printf "$get_photos_gallery_dir_txt_0002" "$gallery_dir")" "CLI" "get_photos_gallery_dir"
    if create_directory "$gallery_dir"; then
        if [ -n "$event_name" ]; then
            local event_dir="$gallery_dir/$event_name"
            debug "$(printf "$get_photos_gallery_dir_txt_0003" "$event_dir")" "CLI" "get_photos_gallery_dir"
            if create_directory "$event_dir"; then
                debug "$(printf "$get_photos_gallery_dir_txt_0004" "$event_dir")" "CLI" "get_photos_gallery_dir"
                echo "$event_dir"
                return 0
            fi
        else
            debug "$(printf "$get_photos_gallery_dir_txt_0005" "$gallery_dir")" "CLI" "get_photos_gallery_dir"
            echo "$gallery_dir"
            return 0
        fi
    fi
    ...
    # Fallback, wenn Event-Verzeichnis nicht erstellt werden konnte
    debug "$(printf "$get_photos_gallery_dir_txt_0006" "$photos_dir")" "CLI" "get_photos_gallery_dir"
    echo "$photos_dir"
    return 0
}

# get_frontend_picture_dir
get_frontend_picture_dir_txt_0001="Ermittle Frontend-Picture-Verzeichnis"
get_frontend_picture_dir_txt_0002="Prüfe Picture-Verzeichnis: %s"
get_frontend_picture_dir_txt_0003="Verwende Picture-Verzeichnis: %s"
get_frontend_picture_dir_txt_0004="Picture-Verzeichnis nicht verfügbar, verwende Fallback: %s"
...
get_frontend_picture_dir() {
    ...
    # -----------------------------------------------------------------------
    local frontend_dir
    debug "$get_frontend_picture_dir_txt_0001" "CLI" "get_frontend_picture_dir"
    
    # Hole das Frontend-Hauptverzeichnis
    frontend_dir=$(get_frontend_dir)
    local picture_dir="$frontend_dir/picture"
    
    debug "$(printf "$get_frontend_picture_dir_txt_0002" "$picture_dir")" "CLI" "get_frontend_picture_dir"
    if create_directory "$picture_dir"; then
        debug "$(printf "$get_frontend_picture_dir_txt_0003" "$picture_dir")" "CLI" "get_frontend_picture_dir"
        echo "$picture_dir"
        return 0
    fi
    
    # Fallback direkt zum Frontend-Verzeichnis
    debug "$(printf "$get_frontend_picture_dir_txt_0004" "$frontend_dir")" "CLI" "get_frontend_picture_dir"
    echo "$frontend_dir"
    return 1
}
...
# get_log_dir
get_log_dir_txt_0001="Ermittle Log-Verzeichnis"
get_log_dir_txt_0002="Verwende bereits definiertes LOG_DIR: %s"
get_log_dir_txt_0003="Prüfe Standard-Logverzeichnis: %s"
get_log_dir_txt_0004="Verwende Standard-Logverzeichnis: %s"
get_log_dir_txt_0005="Standard-Logverzeichnis nicht verfügbar, prüfe Fallback-Optionen"
get_log_dir_txt_0006="Verwende Fallback 1: %s"
get_log_dir_txt_0007="Verwende Fallback 2: %s"
get_log_dir_txt_0008="Verwende Fallback 3: %s"
get_log_dir_txt_0009="Fehler: Keine Schreibrechte im Logverzeichnis %s"
get_log_dir_txt_0010="Fehler: Auch Fallback-Logverzeichnis %s nicht schreibbar"
get_log_dir_txt_0011="Fehler: Kein schreibbares Logverzeichnis gefunden"
get_log_dir_txt_0012="Warnung: Logverzeichnis konnte nicht erstellt werden, verwende aktuelles Verzeichnis"
get_log_dir_txt_0013="Entferne vorhandenes Verzeichnis /var/log/fotobox"
get_log_dir_txt_0014="Symlink /var/log/fotobox zeigt auf %s, wird auf %s geändert"
get_log_dir_txt_0015="Symlink /var/log/fotobox zeigt bereits korrekt auf %s"
get_log_dir_txt_0016="Erstelle Symlink in /var/log/fotobox zu %s"
...
get_log_dir() {
    ...
    debug "$get_log_dir_txt_0001" "CLI" "get_log_dir"
    if [ -n "$LOG_DIR" ] && [ -d "$LOG_DIR" ]; then
        debug "$(printf "$get_log_dir_txt_0002" "$LOG_DIR")" "CLI" "get_log_dir" >/dev/null 2>&1
    ...
    if [ $found_dir -eq 0 ]; then
        ...
        debug "$(printf "$get_log_dir_txt_0003" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
        if [ -d "$logdir" ] || mkdir -p "$logdir" 2>/dev/null; then
            create_directory "$logdir" || true
            debug "$(printf "$get_log_dir_txt_0004" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
            found_dir=1
        fi
    fi
    ...
    if [ $found_dir -eq 0 ]; then
        debug "$get_log_dir_txt_0005" "CLI" "get_log_dir" >/dev/null 2>&1
        if [ -w "/var/log" ]; then
            logdir="$FALLBACK_DIR_LOG"
            debug "$(printf "$get_log_dir_txt_0006" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
            create_directory "$logdir" || true
            found_dir=1
        elif [ -w "/tmp" ]; then
            logdir="$FALLBACK_DIR_LOG_2"
            debug "$(printf "$get_log_dir_txt_0007" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
            create_directory "$logdir" || true
            found_dir=1
        else
            logdir="$FALLBACK_DIR_LOG_3"
            debug "$(printf "$get_log_dir_txt_0008" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
            create_directory "$logdir" || true
            found_dir=1
        fi
    fi
    ...
    if [ "$(id -u)" = "0" ] && [ -w "/var/log" ]; then
        ...
        if [ -d "/var/log/fotobox" ] && [ ! -L "/var/log/fotobox" ]; then
            debug "$get_log_dir_txt_0013" "CLI" "get_log_dir" >/dev/null 2>&1
        ...
        if [ -L "/var/log/fotobox" ]; then
            ...
            if [ "$current_target" != "$logdir" ]; then
                debug "$(printf "$get_log_dir_txt_0014" "$current_target" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
                rm -f /var/log/fotobox 2>/dev/null || true
            else
                debug "$(printf "$get_log_dir_txt_0015" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
                echo "$logdir"
                return 0
            fi
        fi
        
        debug "$(printf "$get_log_dir_txt_0016" "$logdir")" "CLI" "get_log_dir" >/dev/null 2>&1
...
# get_tmp_dir
get_tmp_dir_txt_0001="Ermittle temporäres Verzeichnis"
get_tmp_dir_txt_0002="Verwende bereits definiertes TMP_DIR: %s"
get_tmp_dir_txt_0003="Prüfe Standard-Temp-Verzeichnis: %s"
get_tmp_dir_txt_0004="Verwende Standard-Temp-Verzeichnis: %s"
get_tmp_dir_txt_0005="Standard-Temp-Verzeichnis nicht verfügbar, prüfe Fallback-Optionen"
get_tmp_dir_txt_0006="Verwende Fallback im Log-Verzeichnis: %s"
get_tmp_dir_txt_0007="Verwende Fallback-System-Temp: %s"
get_tmp_dir_txt_0008="Verwende Systemverzeichnis /tmp als letzte Option"
get_tmp_dir_txt_0009="Entferne vorhandenes Verzeichnis /tmp/fotobox"
get_tmp_dir_txt_0010="Symlink /tmp/fotobox zeigt auf %s, wird auf %s geändert"
get_tmp_dir_txt_0011="Symlink /tmp/fotobox zeigt bereits korrekt auf %s"
get_tmp_dir_txt_0012="Erstelle Symlink in /tmp/fotobox zu %s"
...
get_tmp_dir() {
    ...
    debug "$get_tmp_dir_txt_0001" "CLI" "get_tmp_dir" >/dev/null 2>&1
    
    # Prüfen, ob TMP_DIR bereits gesetzt ist (z.B. vom install.sh)
    if [ -n "$TMP_DIR" ] && [ -d "$TMP_DIR" ]; then
        debug "$(printf "$get_tmp_dir_txt_0002" "$TMP_DIR")" "CLI" "get_tmp_dir" >/dev/null 2>&1
    ...
    if [ $found_dir -eq 0 ]; then
        ...
        debug "$(printf "$get_tmp_dir_txt_0003" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
        
        if [ -d "$tmpdir" ] || mkdir -p "$tmpdir" 2>/dev/null; then
            create_directory "$tmpdir" || true
            debug "$(printf "$get_tmp_dir_txt_0004" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
            found_dir=1
        fi
    fi
    ...
    if [ $found_dir -eq 0 ]; then
        debug "$get_tmp_dir_txt_0005" "CLI" "get_tmp_dir" >/dev/null 2>&1
        
        # Fallback auf das Projekt-Log-Verzeichnis
        tmpdir="$(get_log_dir)/tmp"
        if mkdir -p "$tmpdir" 2>/dev/null; then
            debug "$(printf "$get_tmp_dir_txt_0006" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
            create_directory "$tmpdir" || true
            found_dir=1
        fi
    fi
    ...
    if [ $found_dir -eq 0 ]; then
        ...
        if mkdir -p "$tmpdir" 2>/dev/null; then
            debug "$(printf "$get_tmp_dir_txt_0007" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
            create_directory "$tmpdir" || true
            found_dir=1
        fi
    fi
    ...
    if [ $found_dir -eq 0 ]; then
        ...
        tmpdir="/tmp"
        debug "$get_tmp_dir_txt_0008" "CLI" "get_tmp_dir" >/dev/null 2>&1
        found_dir=1
    fi
    ...
    if [ "$(id -u)" = "0" ] && [ -w "/tmp" ]; then
        ...
        if [ -d "/tmp/fotobox" ] && [ ! -L "/tmp/fotobox" ]; then
            debug "$get_tmp_dir_txt_0009" "CLI" "get_tmp_dir" >/dev/null 2>&1
        ...
        if [ -L "/tmp/fotobox" ]; then
            ...
            if [ "$current_target" != "$tmpdir" ]; then
                debug "$(printf "$get_tmp_dir_txt_0010" "$current_target" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
                rm -f /tmp/fotobox 2>/dev/null || true
            else
                debug "$(printf "$get_tmp_dir_txt_0011" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
                echo "$tmpdir"
                return 0
            fi
        fi
        
        debug "$(printf "$get_tmp_dir_txt_0012" "$tmpdir")" "CLI" "get_tmp_dir" >/dev/null 2>&1
...
ensure_folder_structure_txt_0001="Stelle sicher, dass alle notwendigen Verzeichnisse existieren"
ensure_folder_structure_txt_0002="Ordnerstruktur erfolgreich erstellt"
...
ensure_folder_structure() {
    ...
    # -----------------------------------------------------------------------
    debug "$ensure_folder_structure_txt_0001" "CLI" "ensure_folder_structure"
    ...
    debug "$ensure_folder_structure_txt_0002" "CLI" "ensure_folder_structure"
    return 0
}
